<template>
    <a-card :bordered="false">
        ...
        <a-space>
            ...
            <a-button type="primary" v-auth:order-manage="'batch_modify_invoice_address'">测试按钮权限</a-button>  <!--v-auth:菜单名="权限值"-->
...
<script setup lang="ts">
    ...
    const testModal = () => {
        useModal(test,{info:{text:'haha'}},{title:t('dialogTitle'),width:800}).then((res)=>{
            console.log(res)
        }).catch((err)=>{
            console.log(err)
        })
